...
1. **Invoice MCP Server (Python)** — exposes *workflow* MCP tools:
   ...
   - `payment_prepare` — infer/payee+funding method+currency+purpose and produce a *draft* requiring minimal user input
   - `payment_submit` — create/submit the payment in Veem
   - `payment_schedule` — schedule a payment (POC uses SQLite persistence; swap to PaymentDomain schedule API in prod)
...
> ⚠️ This is a **refactor / re-layout** of an existing POC code dump. The goal is to make the codebase easy to reason about,
> keep MCP tools **E2E & workflow-oriented**, and make testing straightforward.
...
- **Pattern A: “Full E2E workflow tools”**  
  Each MCP tool is a complete workflow step (parse → infer → validate → draft/submit),
  rather than a 1:1 wrapper around Veem’s REST endpoints.

- **Ports & adapters**  
  All external dependencies are behind interfaces (Veem API, history store, schedule store, LLM extractor)
  so we can unit-test deterministically.

- **Testing**  
  We use `pytest` + `pytest-asyncio` for unit/contract/integration/E2E tests. See `apps/invoice-mcp-server/README.md`.
